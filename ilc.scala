...
case class Num(n: Int) extends Exp { override def toString = n.toString}
case class Add(e1: Exp, e2: Exp) extends Exp { override def toString = e1.toString + " + " + e2.toString }
...
// this code is so schematic that it could mostly be generated e.g. as a datatype-generic algorithm
// the "Case" part could presumably be optimized a little more, though

def map0(e: Exp, f: Exp => Exp, booleanTag: Exp) : Exp = {
  Pair(f(Proj1(e)), booleanTag)
}

def map1(e: Exp, f: Exp => Exp) : Exp = {
  val x = freshName
  let(x, e, Pair(f(Proj1(x)), Proj2(x)))
}
def map2(e1: Exp, e2: Exp, f: (Exp,Exp) => Exp) : Exp = {
  val x = freshName
  val y = freshName
  let(x, e1,
   let(y, e2, Pair(f(Proj1(x), Proj1(y)), or(Proj2(x),Proj2(y)))))
}
...
def map3(e1: Exp, e2: Exp, e3: Exp, f: (Exp,Exp,Exp) => Exp) : Exp = {
  ...
  let(x, e1,
   let(y, e2,
    let(z,e3, Pair(f(Proj1(x), Proj1(y), Proj1(z)), or(or(Proj2(x),Proj2(y)), Proj2(z))))))
}
...
def derive(e: Exp) : Exp = e match {
  ...
  case Add(e1,e2) => map2(derive(e1),derive(e2), { case (de1,de2) => Add(de1,de2) })
...
def eval(e: Exp, env: Map[Symbol, Value]) : Value = e match {
  ...
  case App(e1,e2) => eval(e1,env) match {
                        case ClosureV(f,env2) => eval(f.e, Map(f.x -> eval(e2,env)))
						case other => sys.error("Closure expected, found: "+other.toString)
					}
  ...
  case Proj1(e) => eval(e,env) match { case PairV(e1,_) => e1; case other => sys.error("Pair expected, found: "+other.toString) }
  case Proj2(e) => eval(e,env) match { case PairV(_,e2) => e2; case other => sys.error("Pair expected, found: "+other.toString) }
