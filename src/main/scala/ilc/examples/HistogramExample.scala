...
//  val OLDuserMap = lambda(ℤ) { ignoredDocumentID =>
//    FoldGroup ! FreeAbelianGroup(ProductType(ℤ, ℤ)) !
//      lambda(ℤ) { number => Singleton ! (Pair ! number ! LiteralInt(1)) }
//  }

  val userMap: UntypedTerm = 'ignored ->: foldGroup(freeAbelianGroup,
...
//  val OLDuserReduce = lambda(ℤ) { ignoredKey =>
//    FoldGroup ! additiveGroupOnIntegers ! lambda(ℤ) { x => x }
//  }

  val userReduce = 'ignored ->: foldGroup(additiveGroupOnIntegers, 'x ->: 'x)
